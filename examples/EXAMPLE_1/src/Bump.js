export default class Bump {
    ...
    hitTestPoint(point, sprite) {
        ...
        if (shape === "circle") {
            ...
            let dx = point.x - sprite.x - (sprite.width / 2) + sprite.xAnchorOffset,
                dy = point.y - sprite.y - (sprite.height / 2) + sprite.yAnchorOffset,
    ...
    hitTestCircle(c1, c2, global = false) {
        ...
        let vx, dy, magnitude, combinedRadii, hit;

        //Calculate the vector between the circles’ center points
        if (global) {
            //Use global coordinates
            dx = (c2.gx + (c2.width / 2) - c2.xAnchorOffset) - (c1.gx + (c1.width / 2) - c1.xAnchorOffset);
            dy = (c2.gy + (c2.width / 2) - c2.yAnchorOffset) - (c1.gy + (c1.width / 2) - c1.yAnchorOffset);
        } else {
            //Use local coordinates
            dx = (c2.x + (c2.width / 2) - c2.xAnchorOffset) - (c1.x + (c1.width / 2) - c1.xAnchorOffset);
            dy = (c2.y + (c2.width / 2) - c2.yAnchorOffset) - (c1.y + (c1.width / 2) - c1.yAnchorOffset);
        }
    ...
    movingCircleCollision(c1, c2, global = false) {
        ...
        if (global) {
            ...
            //Use global coordinates
            s.dx = (c2.gx + c2.radius - c2.xAnchorOffset) - (c1.gx + c1.radius - c1.xAnchorOffset);
            s.dy = (c2.gy + c2.radius - c2.yAnchorOffset) - (c1.gy + c1.radius - c1.yAnchorOffset);
        } else {

            //Use local coordinates
            s.dx = (c2.x + c2.radius - c2.xAnchorOffset) - (c1.x + c1.radius - c1.xAnchorOffset);
            s.dy = (c2.y + c2.radius - c2.yAnchorOffset) - (c1.y + c1.radius - c1.yAnchorOffset);
        }

        //Find the distance between the circles by calculating
        //the vector's magnitude (how long the vector is)
        s.magnitude = Math.sqrt(s.dx * s.dx + s.dy * s.dy);
        ...
        if (s.magnitude < combinedRadii) {
            ...
            //Normalize the vector.
            //These numbers tell us the direction of the collision
            s.dx = s.dx / s.magnitude;
            s.dy = s.dy / s.magnitude;

            //Find the collision vector.
            //Divide it in half to share between the circles, and make it absolute
            s.dxHalf = Math.abs(s.dx * overlap / 2);
            s.dyHalf = Math.abs(s.dy * overlap / 2);
            ...
            c1.x = c1.x + (s.dxHalf * xSide);
            c1.y = c1.y + (s.dyHalf * ySide);

            //Move c2 out of the collision
            c2.x = c2.x + (s.dxHalf * -xSide);
            c2.y = c2.y + (s.dyHalf * -ySide);
            ...
            let dp2 = c1.dx * (s.lx / s.magnitude) + c1.dy * (s.ly / s.magnitude);
            ...
            let dp4 = c2.dx * (s.lx / s.magnitude) + c2.dy * (s.ly / s.magnitude);
            ...
            c1.dx = c1.bounce.x / c1.mass;
            c1.dy = c1.bounce.y / c1.mass;
            c2.dx = c2.bounce.x / c2.mass;
            c2.dy = c2.bounce.y / c2.mass;
        }
        return hit;
    }
    ...
    rectangleCollision(r1, r2, bounce = false, global = true) {
        ...
        if (global) {
            dx = (r1.gx + Math.abs(r1.halfWidth) - r1.xAnchorOffset) - (r2.gx + Math.abs(r2.halfWidth) - r2.xAnchorOffset);
            dy = (r1.gy + Math.abs(r1.halfHeight) - r1.yAnchorOffset) - (r2.gy + Math.abs(r2.halfHeight) - r2.yAnchorOffset);
        } else {
            //dx = r1.centerX - r2.centerX;
            //dy = r1.centerY - r2.centerY;
            dx = (r1.x + Math.abs(r1.halfWidth) - r1.xAnchorOffset) - (r2.x + Math.abs(r2.halfWidth) - r2.xAnchorOffset);
            dy = (r1.y + Math.abs(r1.halfHeight) - r1.yAnchorOffset) - (r2.y + Math.abs(r2.halfHeight) - r2.yAnchorOffset);
        }
    ...
    bounceOffSurface(o, s) {
        ...
        dp2 = o.dx * (s.lx / s.magnitude) + o.dy * (s.ly / s.magnitude);

        //Project the object's velocity onto the surface's left normal
        p2.dx = dp2 * (s.lx / s.magnitude);
        p2.dy = dp2 * (s.ly / s.magnitude);
        ...
        bounce.x = p1.dx + p2.dx;
        bounce.y = p1.dy + p2.dy;
    ...
    /*
     ...
     if (bounce) sprite.dx *= -1;

     //If the sprite has `mass`, let the mass
     //affect the sprite's velocity
     if(sprite.mass) sprite.dx /= sprite.mass;
     ...
     if (bounce) sprite.dy *= -1;
     if(sprite.mass) sprite.dy /= sprite.mass;
